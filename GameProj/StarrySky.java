...
public class StarrySky implements Background
{
    ...
    public Image getImage()
    {
        ...
        editor.setColor(backgroundColor);
        for (Star star : stars)
            star.drawMe(editor);
        return returnImage;
    }
    ...
    private class Star implements Comparable<Star>
    {
        ...
        public void drawMe(Graphics g)
        {
            ...
            switch (depth) {
                ...
                for (int i = 0; i < 4; i++)
                    g.fillRect(x+5-(i/2)*5,y+5-(i%2)*5,1,1);
                break;
                case 0:
                g.fillRect(x,y,9,9);
                g.setColor(backgroundColor);
                for (int i = 0; i < 4; i++)
                {
                    g.fillRect(x+8-(i/2)*8,y+8-(i%2)*8,1,1);
                    g.fillRect(x+7-(i/2)*6,y+8-(i%2)*8,1,1);
                    g.fillRect(x+8-(i/2)*8,y+7-(i%2)*6,1,1);
                }
            }
        }
    }
